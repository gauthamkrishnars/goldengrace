  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 mb-6">
        ...
        {category && <p className="text-sm text-gray-500 mt-1">{category.description}</p>}
              ...
              {metalFilters.map((metal) => (
                <button key={metal} onClick={() => setSelectedMetal(metal)}
                  className={`px-3 py-1.5 text-xs rounded-full border transition-colors ${
                    selectedMetal === metal ? "bg-brand text-white border-brand" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                  }`}>
                  {metal}
                </button>
              ))}
      ...
      <div className="max-w-7xl mx-auto px-4 pb-12">
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => setSelectedMetal("All")} className="mt-3 text-brand text-sm font-medium hover:underline">
              Clear Filters
            </button>
          </div>
        )}
