...
import { Heart, Truck, Shield, RotateCcw, ChevronRight, Star, Minus, Plus, Check } from "lucide-react";
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="flex-1 flex items-center justify-center py-32">
          <div className="text-center">
            ...
            <Link href="/" className="mt-4 inline-block text-brand font-medium hover:underline">
              Go Home
            </Link>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="relative aspect-square bg-gray-50 rounded-2xl overflow-hidden">
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-1">
                {product.isNew && (
                  <span className="bg-brand text-white text-xs font-bold px-2.5 py-1 rounded-full">NEW</span>
                )}
                {product.isBestseller && (
                  <span className="bg-amber-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">BESTSELLER</span>
                )}
                {product.discount && (
                  <span className="bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">{product.discount}% OFF</span>
                )}
              </div>
            </div>
              ...
              {product.images.map((img, i) => (
                <button
                  ...
                  className={`relative w-16 h-16 md:w-20 md:h-20 rounded-lg overflow-hidden border-2 transition-colors ${
                    selectedImage === i ? "border-brand" : "border-gray-100 hover:border-gray-200"
                  }`}
                >
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < Math.floor(product.rating) ? "text-amber-400 fill-amber-400" : "text-gray-200"}`}
                  />
                ))}
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center border border-gray-200 rounded-lg">
                ...
                <span className="px-4 text-sm font-semibold text-gray-800 min-w-[2rem] text-center">
                  {quantity}
                </span>
                <button
            ...
            <div className="grid grid-cols-3 gap-3 pt-2">
              <div className="text-center">
                <Truck className="h-5 w-5 text-brand mx-auto mb-1" />
                <p className="text-[10px] text-gray-500">Free Shipping</p>
              </div>
              <div className="text-center">
                <Shield className="h-5 w-5 text-brand mx-auto mb-1" />
                <p className="text-[10px] text-gray-500">Lifetime Exchange</p>
              </div>
              <div className="text-center">
            ...
            <div className="border-t border-gray-100 pt-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between py-1.5 bg-gray-50 px-2 rounded">
                  ...
                  <span className="text-gray-700 font-medium">{product.metal}</span>
                </div>
                <div className="flex justify-between py-1.5 bg-gray-50 px-2 rounded">
                  <span className="text-gray-500">Metal Weight</span>
                  <span className="text-gray-700 font-medium">{product.metalWeight}</span>
                </div>
                {product.stone && (
                  <div className="flex justify-between py-1.5 bg-gray-50 px-2 rounded">
                    <span className="text-gray-500">Stone</span>
                    <span className="text-gray-700 font-medium">{product.stone}</span>
                  </div>
                )}
                {product.stoneWeight && (
                  <div className="flex justify-between py-1.5 bg-gray-50 px-2 rounded">
                    <span className="text-gray-500">Stone Weight</span>
                    <span className="text-gray-700 font-medium">{product.stoneWeight}</span>
                  </div>
                )}
                <div className="flex justify-between py-1.5 bg-gray-50 px-2 rounded col-span-2">
